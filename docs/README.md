...
## クラス図及びシーケンス図
簡易的な[クラス図](module/class.drawio)と、[シーケンス図](module/sequence.drawio)を作成している。これらを更新しながら開発ヲ行う。

## 現状までの実装
[windows_base.sln](../windows_base/windows_base.sln)から`integration_testプロジェクト`を実行することでアプリケーションを実行できる。各ライブラリらを使用した統合テストの役割も持つ。Facadeパターンを使用することで利用しやすく、また良い結合度になるように設計している。
